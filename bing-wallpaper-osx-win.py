...
def set_osx_wallpaper(file_path):
    if os.path.isfile(file_path):
        import subprocess
        subprocess.Popen(SCRIPT.format(file_path=file_path), shell=True)
...
def download_wallpaper(idx=0, use_wallpaper=True):
    ...
    if os.path.isfile(pic_path):
        # If today's file exists, don't do anything at all.
        # We've already downloaded it, we've already set it as the wallpaper,
        # and we've already deleted old wallpapers.
        print('Image of', date.strftime('%d-%m-%Y'), 'already downloaded.')
        return
    # Otherwise, download the new file, set it, and delete old ones.
    ...
    for other_pic in os.listdir(save_dir):
        full_path_other_pic = join_path(save_dir, other_pic)
        if os.path.isfile(full_path_other_pic) and full_path_other_pic != pic_path:
            print(f'Deleting old wallpaper: {other_pic}')
            os.remove(full_path_other_pic)
